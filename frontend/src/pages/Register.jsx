// Registration page for a regular (non-admin) user.
// Mock mode: registerUser() just saves the new account into the browser's
// localStorage (see auth.js). There is no real backend behind this yet.
import { useState } from 'react'
import { Link, useNavigate } from 'react-router-dom'
import { allowedEmail, registerUser, startSession } from '../auth.js'
...
function Register() {
  ...
  const handleSubmit = (e) => {
    ...
    // Basic form validation, checked in order so only one message shows at a time.
    ...
    const created = registerUser({ name: fullName.trim(), email: email.trim(), password })
    if (!created) {
    ...
    setError('')
    // Registering also logs the person in, same as most real apps do.
    startSession(email.trim(), 'user')
    navigate('/dashboard')
  }
      ...
      <div className="mx-auto flex w-full max-w-md flex-col justify-center lg:max-w-lg">
        ...
        <div className="rounded-3xl border border-slate-200 bg-white p-6 shadow-[0_18px_55px_rgba(16,42,67,0.08)] sm:p-10">
          ...
          <div className="mt-8 text-center text-sm text-slate-500">
            Already have an account? <Link className="font-bold text-[#117d72] hover:text-[#102a43]" to="/login">Sign In</Link>
          </div>
        </div>
      </div>
    </div>
