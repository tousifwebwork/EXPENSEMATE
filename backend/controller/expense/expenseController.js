  ...
  try {
    ...
    if (typeof shares === "string") {
      try {
        ...
        return res.status(400).json({ success: false, message: "Invalid shares data" });
      }
    }
    ...
    if (splitType !== "fullPayment" && splitType !== "none") {
      const invalidParticipant = shares.find(
        (share) => !share.user || !memberIds.includes(share.user.toString())
      );
    ...
    if (splitType === "equal") {
      ...
      finalShares = calculateEqualSplit(Number(amount), participantIds);
      ...
      finalShares = shares.map((share) => ({ user: share.user, amount: Number(share.amount) }));

    } else if (splitType === "percentage") {
      if (shares.length === 0) {
        return res.status(400).json({ success: false, message: "Select at least one participant" });
      }
      finalShares = calculatePercentageSplit(Number(amount), shares);
      if (!finalShares) {
...
exports.updateExpense = async (req, res) => {
  try {
    ...
    // ✅ Capture OLD values before any changes — needed for activity log
    ...
    if (amount !== undefined || splitType !== undefined || shares !== undefined) {
      ...
      let finalRawShares = shares !== undefined ? shares : expense.shares;

      if (!Array.isArray(finalRawShares)) {
        return res.status(400).json({ success: false, message: "Shares must be an array" });
      }
      ...
      if (finalSplitType === "equal") {
        const participantIds = finalRawShares.map((share) => share.user);
        if (participantIds.length === 0) {
          return res.status(400).json({ success: false, message: "Select at least one participant" });
        }
        ...
        finalShares = finalRawShares.map((share) => ({ user: share.user, amount: Number(share.amount) }));
    ...
    // ✅ Log activity AFTER save, using captured old values
    await logActivity({
      group: expense.group,
      actor: userId,
      action: "expense_updated",
      description: `updated expense "${expense.title}"`,
      metadata: { oldAmount, newAmount: expense.amount, oldTitle, newTitle: expense.title },
    });

    return res.status(200).json({ success: true, message: "Expense updated", expense });
